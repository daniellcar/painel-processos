...
export function SessionAlertOverlay({
  ...
  return (
    <motion.div
      ...
      onClick={onDismiss}
      className="fixed inset-0 z-[100] flex cursor-pointer flex-col items-center justify-center bg-[--color-paper]"
    >
      {/* Grain texture overlay */}
      <div className="pointer-events-none absolute inset-0 grain" />

      {/* Top banner — heavy ink stripe */}
      <motion.div
        initial={{ y: -120, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.55, ease: EASE }}
        className="absolute top-0 left-0 right-0 z-10 border-b-[3px] border-[--color-claret] bg-[--color-ink]"
      >
        <div className="flex items-center justify-between px-12 py-4">
          <div className="flex items-center gap-4">
            <span className="relative inline-block h-2 w-2 rounded-full bg-[--color-claret]">
              <span className="live-dot absolute inset-0 rounded-full" />
            </span>
            <span className="font-sans text-xs font-bold uppercase tracking-[0.45em] text-[--color-paper]">
          ...
          <span className="font-mono text-xs uppercase tracking-[0.35em] text-[--color-paper-warm]">
            {dateLabel}
          </span>
        </div>
      </motion.div>
      ...
      >
        ...
        >
          <span className="font-sans text-[10px] font-semibold uppercase tracking-[0.5em] text-[--color-claret]">
            Hora marcada
          </span>
          <span className="font-serif text-7xl tabular leading-none text-[--color-ink] sm:text-8xl">
            {sessionTime.replace(":", " : ")}
          </span>
        </motion.div>
        ...
        <motion.h1
          ...
          className="mt-6 text-center font-serif italic leading-[0.85] text-[--color-ink]"
          style={{ fontSize: "clamp(6rem, 14vw, 11rem)" }}
        >
          Sessão <em className="text-[--color-claret]">aberta.</em>
        </motion.h1>
        ...
        <motion.div
          ...
          className="mt-12 w-full max-w-3xl border-y-[3px] border-double border-[--color-ink] px-10 py-7"
        >
          <dl className="grid grid-cols-[140px_1fr] gap-x-8 gap-y-4 text-left">
      ...
      {/* Bottom banner — countdown + dismiss hint */}
      <motion.div
        initial={{ y: 120, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.55, ease: EASE, delay: 0.1 }}
        className="absolute bottom-0 left-0 right-0 border-t-[3px] border-[--color-claret] bg-[--color-ink]"
      >
        <div className="flex items-center justify-between px-12 py-4">
          ...
          <div className="flex items-baseline gap-3">
            ...
            <span className="font-mono text-2xl font-semibold tabular text-[--color-paper]">
